...
describe('根式馬戲團免登入試玩', () => {
  it('同類方根出牌後補牌，並進入馴化步驟', () => {
    const room = submitDemoPair(createAnimalEquationDemo(), ['sqrt8', 'sqrt18'])
  ...
  it('不同類方根被 AI 抓錯，不棄牌且可以重試', () => {
    const room = submitDemoPair(createAnimalEquationDemo(), ['sqrt8', 'sqrt3'])
  ...
  it('n 牌必須先輸入正整數', () => {
    const initial = createAnimalEquationDemo()
  ...
  it('馴化翻開動物並以真實分值更新得分', () => {
    const room = submitDemoPair(createAnimalEquationDemo(), ['sqrt8', 'sqrt18'])
    const finished = tameDemoAnimal(room, 'deer')
    expect(finished.phase).toBe('complete')
    expect(finished.players[0]).toMatchObject({ animalCount: 1, animalScore: 20 })
    expect(finished.animals.find((animal) => animal.id === 'deer')).toMatchObject({ revealed: true, ownerPlayerId: 'you' })
    expect(tameDemoAnimal(finished, 'octopus')).toBe(finished)
  })
})
