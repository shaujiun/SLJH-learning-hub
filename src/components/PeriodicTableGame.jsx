...
function PeriodicTableGrid({ allowedNumbers, targetNumber, selectedNumber, answered, onSelect }) {
    ...
    <div className="periodic-table-scroll" aria-label="元素週期表作答區">
      <div className="periodic-table-grid" role="grid">
        ...
        {Array.from({ length: 18 }, (_, index) => (
          <span className="group-label" style={{ gridColumn: index + 2, gridRow: 1 }} key={`group-${index + 1}`}>
            {index + 1}
          </span>
        ))}
        {Array.from({ length: 7 }, (_, index) => (
          <span className="period-label" style={{ gridColumn: 1, gridRow: index + 2 }} key={`period-${index + 1}`}>
            {index + 1}
          </span>
        ))}
        {mainElements.map((element) => renderElement(element, {
        ...
        {actinides.map((element, index) => renderElement(element, {
          gridColumn: index + 4,
          gridRow: 10,
        }))}
      </div>
